...
import {
  AlertTriangle, BarChart3, Bell, BookOpen, CalendarDays, CheckCircle2, ChevronDown, CircleHelp, ClipboardList, ExternalLink,
  FileText, GraduationCap, LoaderCircle, MapPin, Maximize2, MessageSquareText, Mic, Pencil, Plus, RefreshCw, RotateCcw,
  Save, Send, Sparkles, Square, Trash2, Upload, UserPlus, X, XCircle,
} from 'lucide-react';
import {
  addLearningAsset, addLearningMember, createLearningCalendarItem, createLearningCourse, createLearningQuestion,
  createLearningAssignment, createSession, deleteLearningArtifact, deleteLearningAssignment, deleteLearningCalendarItem, deleteLearningCourse,
  ...
  updateLearningAsset, updateLearningCalendarItem, updateLearningCourse, updateLearningLessonProgress, updateLearningQuestion,
  updateLearningMemberProfile,
} from '../services/api.js';
import MarkdownRenderer from './MarkdownRenderer.jsx';

const TABS = [
  ['overview', 'Overview'], ['calendar', 'Calendar'], ['curriculum', 'Curriculum'], ['content', 'Course Content'],
      ...
      {!workspaceId ? <Empty title="Select a workspace" text="Learning courses are governed by a DocIntel workspace. Select or create one first."/> : <>
        ...
        {!course ? <Empty title="Build your first course" text="Create a course, attach processed workspace content, and begin grounded learning."/> : <>
          ...
          <main style={s.body}>
            ...
            {tab==='calendar' && <CourseCalendar course={course} mutate={mutate}/>}
...
function Overview({course,setCourse,mutate,onDeleted}) {
  ...
  const [profile,setProfile]=useState(course.my_profile||{});
  useEffect(()=>setProfile(course.my_profile||{}),[course.id,course.my_profile]);
  ...
  const saveProfile=()=>mutate(()=>updateLearningMemberProfile(course.id,{
    headline:profile.headline||'',bio:profile.bio||'',skills:splitProfileList(profile.skills),
    interests:splitProfileList(profile.interests),city:profile.city||'',region:profile.region||'',
    country:profile.country||'',timezone:profile.timezone||'',directory_visible:profile.directory_visible!==false,
  }));
  ...
  {course.my_profile&&<Section title="My course registration profile" icon={<GraduationCap size={16}/>}><p>Your profile is shared only within this course. Location is limited to city, region, country, and time zone.</p><div style={s.grid}>
    <Field label="Professional or learning headline" wide><input value={profile.headline||''} onChange={e=>setProfile({...profile,headline:e.target.value})} placeholder="What are you learning or working toward?"/></Field>
    <Field label="About me" wide><ExpandableEditor title="Course profile biography" value={profile.bio||''} onChange={value=>setProfile({...profile,bio:value})} placeholder="Introduce yourself to your classmates."/></Field>
    <Field label="Skills"><input value={profileListText(profile.skills)} onChange={e=>setProfile({...profile,skills:e.target.value})} placeholder="Python, research, design"/></Field>
    <Field label="Interests"><input value={profileListText(profile.interests)} onChange={e=>setProfile({...profile,interests:e.target.value})} placeholder="RAG, governance, healthcare"/></Field>
    <Field label="City"><input value={profile.city||''} onChange={e=>setProfile({...profile,city:e.target.value})}/></Field>
    <Field label="State / region"><input value={profile.region||''} onChange={e=>setProfile({...profile,region:e.target.value})}/></Field>
    <Field label="Country"><input value={profile.country||''} onChange={e=>setProfile({...profile,country:e.target.value})}/></Field>
    <Field label="Time zone"><input value={profile.timezone||''} onChange={e=>setProfile({...profile,timezone:e.target.value})} placeholder="America/Los_Angeles"/></Field>
    <Field label="Class directory"><label style={s.profileToggle}><input type="checkbox" checked={profile.directory_visible!==false} onChange={e=>setProfile({...profile,directory_visible:e.target.checked})}/>Visible to classmates</label></Field>
  </div><button style={s.primary} onClick={saveProfile}><Save size={15}/>Save my profile</button></Section>}
  {course.can_manage&&<Section title="Course enrollment" icon={<UserPlus size={16}/>}><div style={s.addRow}><input placeholder="Member email" value={member.email} onChange={e=>setMember({...member,email:e.target.value})}/><select value={member.persona} onChange={e=>setMember({...member,persona:e.target.value})}>{['student','teacher','advisor','admin'].map(x=><option key={x}>{x}</option>)}</select><button style={s.secondary} disabled={!member.email} onClick={async()=>{await mutate(()=>addLearningMember(course.id,member));setMember({...member,email:''})}}><Plus size={15}/>Enroll</button></div></Section>}
  <Section title="Class directory" icon={<UserPlus size={16}/>}><div style={s.profileGrid}>{(course.members||[]).map(item=><article style={s.profileCard} key={item.user_id}><header style={s.profileHeader}><div style={s.metaStack}><strong>{item.full_name||item.email||'Course member'}</strong><small>{pretty(item.persona)}{item.is_current_user?' · You':''}</small></div>{course.can_manage&&item.user_id!==course.created_by&&<button style={s.dangerIcon} onClick={()=>mutate(()=>removeLearningMember(course.id,item.user_id))} title="Remove"><Trash2 size={15}/></button>}</header>{item.headline&&<b style={s.profileHeadline}>{item.headline}</b>}{item.bio&&<p style={s.profileBio}>{item.bio}</p>}<ProfileTags label="Skills" values={item.skills}/><ProfileTags label="Interests" values={item.interests}/>{profileLocation(item)&&<div style={s.profileLocation}><MapPin size={14}/><span>{profileLocation(item)}</span></div>}{course.can_manage&&item.email&&<small>{item.email}</small>}{course.can_manage&&!item.directory_visible&&<small style={s.privateNote}>Hidden from classmates</small>}</article>)}</div>{!course.members?.length&&<Empty title="No visible classmates" text="Course members who share their profile will appear here."/>}</Section>
...
const emptyCalendarItem=()=>({item_type:'announcement',title:'',description:'',starts_at:'',ends_at:'',all_day:false});
...
const calendarPayload=draft=>({
  item_type:draft.item_type,title:draft.title.trim(),description:draft.description||'',
  starts_at:new Date(draft.starts_at).toISOString(),
  ends_at:draft.ends_at?new Date(draft.ends_at).toISOString():null,all_day:Boolean(draft.all_day),
});

function CourseCalendar({course,mutate}) {
  const [draft,setDraft]=useState(emptyCalendarItem());
  const [editingId,setEditingId]=useState('');
  const items=course.calendar_items||[];
  const save=async()=>{
    if(!draft.title.trim()||!draft.starts_at)return;
    const payload=calendarPayload(draft);
    await mutate(()=>editingId?updateLearningCalendarItem(course.id,editingId,payload):createLearningCalendarItem(course.id,payload));
    setDraft(emptyCalendarItem());setEditingId('');
  };
  const edit=item=>{setEditingId(item.id);setDraft({item_type:item.item_type,title:item.title||'',description:item.description||'',starts_at:localDateTime(item.starts_at),ends_at:localDateTime(item.ends_at),all_day:Boolean(item.all_day)});};
  const cancel=()=>{setEditingId('');setDraft(emptyCalendarItem())};
  return <div style={s.scroll}>{course.can_manage_calendar&&<Section title={editingId?'Edit calendar item':'Add announcement or deadline'} icon={<CalendarDays size={16}/>}><div style={s.grid}>
    <Field label="Type"><select value={draft.item_type} onChange={e=>setDraft({...draft,item_type:e.target.value})}><option value="announcement">Announcement</option><option value="deadline">Deadline</option></select></Field>
    <Field label="Title"><input value={draft.title} onChange={e=>setDraft({...draft,title:e.target.value})} placeholder="Course update or deadline"/></Field>
    <Field label="Starts"><input type="datetime-local" value={draft.starts_at} onChange={e=>setDraft({...draft,starts_at:e.target.value})}/></Field>
    <Field label="Ends (optional)"><input type="datetime-local" value={draft.ends_at} onChange={e=>setDraft({...draft,ends_at:e.target.value})}/></Field>
    <Field label="Calendar display"><label style={s.profileToggle}><input type="checkbox" checked={draft.all_day} onChange={e=>setDraft({...draft,all_day:e.target.checked})}/>All-day item</label></Field>
    <Field label="Details" wide><ExpandableEditor title="Calendar item details" value={draft.description} onChange={value=>setDraft({...draft,description:value})} placeholder="What should students know or complete?"/></Field>
  </div><div style={s.inlineActions}>{editingId&&<button style={s.secondary} onClick={cancel}>Cancel</button>}<button style={s.primary} disabled={!draft.title.trim()||!draft.starts_at} onClick={save}><Save size={14}/>{editingId?'Update':'Publish to course'}</button></div></Section>}
  <Section title="Course calendar" icon={<CalendarDays size={16}/>}><p>Announcements, course deadlines, and published assignment due dates are visible to every enrolled student.</p><div style={s.calendarList}>{items.map(item=><article key={item.id} style={{...s.calendarItem,...(item.item_type==='deadline'?s.deadlineItem:s.announcementItem)}}><div style={s.calendarIcon}>{item.item_type==='deadline'?<CalendarDays size={17}/>:<Bell size={17}/>}</div><div style={s.calendarContent}><header style={s.calendarHeader}><div style={s.metaStack}><strong>{item.title}</strong><small>{item.item_type==='deadline'?'Deadline':'Announcement'} · {new Date(item.starts_at).toLocaleString()}{item.all_day?' · All day':''}</small></div><span style={s.calendarSource}>{item.source==='assignment'?'Assignment':'Course'}</span></header>{item.description&&<p style={s.profileBio}>{item.description}</p>}{item.ends_at&&<small>Ends {new Date(item.ends_at).toLocaleString()}</small>}{item.created_by_name&&<small>Set by {item.created_by_name}</small>}</div>{course.can_manage_calendar&&item.editable&&<div style={s.calendarActions}><button style={s.iconBtn} onClick={()=>edit(item)} title="Edit"><Pencil size={14}/></button><button style={s.dangerIcon} onClick={()=>confirm('Delete this course calendar item?')&&mutate(()=>deleteLearningCalendarItem(course.id,item.id))} title="Delete"><Trash2 size={14}/></button></div>}</article>)}</div>{!items.length&&<Empty title="No course dates yet" text="Announcements and deadlines published by course staff will appear here."/>}</Section></div>;
}

const profileListText=value=>Array.isArray(value)?value.join(', '):(value||'');
const splitProfileList=value=>Array.isArray(value)?value:String(value||'').split(',').map(item=>item.trim()).filter(Boolean);
const profileLocation=item=>[item.city,item.region,item.country,item.timezone].filter(Boolean).join(' · ');
function ProfileTags({label,values}) { const rows=splitProfileList(values);return rows.length?<div style={s.profileTagRow}><small>{label}</small><div style={s.profileTags}>{rows.map(value=><span style={s.profileTag} key={value}>{value}</span>)}</div></div>:null }
...
const s={
  ...
  addRow:{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,190px),1fr))',gap:7,alignItems:'end'},checkRow:{display:'flex',alignItems:'center',gap:16,minHeight:36,flexWrap:'wrap'},cards:{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,280px),1fr))',gap:8},card:{display:'flex',alignItems:'center',justifyContent:'space-between',gap:8,padding:9,border:'1px solid var(--b1)',borderRadius:6,background:'var(--s2)'},metaStack:{display:'flex',flexDirection:'column',gap:4,minWidth:0,lineHeight:1.35},profileToggle:{display:'flex',alignItems:'center',gap:7,minHeight:36},profileGrid:{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,280px),1fr))',gap:9},profileCard:{display:'flex',flexDirection:'column',gap:9,minHeight:150,padding:12,border:'1px solid var(--b1)',borderRadius:7,background:'var(--s2)'},profileHeader:{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:8},profileHeadline:{fontSize:12,color:'#bbf7d0'},profileBio:{margin:0,color:'var(--tx2)',fontSize:11.5,lineHeight:1.5,whiteSpace:'pre-wrap'},profileTagRow:{display:'grid',gap:5},profileTags:{display:'flex',gap:5,flexWrap:'wrap'},profileTag:{padding:'3px 7px',border:'1px solid rgba(74,222,128,.2)',borderRadius:12,background:'rgba(74,222,128,.07)',color:'#bbf7d0',fontSize:10},profileLocation:{display:'flex',alignItems:'center',gap:6,color:'#93c5fd',fontSize:10.5},privateNote:{color:'#fde68a'},lesson:{display:'grid',gridTemplateColumns:'30px minmax(0,1fr) 30px',gap:8,alignItems:'center',marginBottom:9,padding:'8px 0',borderBottom:'1px solid var(--b1)'},lessonNumber:{alignSelf:'start',display:'grid',placeItems:'center',width:25,height:25,borderRadius:5,background:'rgba(74,222,128,.1)',color:'#86efac',fontSize:11,fontWeight:800},lessonFields:{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,220px),1fr))',gap:8,minWidth:0},inlineActions:{display:'flex',justifyContent:'flex-end',gap:6,marginTop:7},toolbar:{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap',marginBottom:10},mappingRow:{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,170px),1fr))',gap:8,alignItems:'end'},
  calendarList:{display:'grid',gap:8},calendarItem:{display:'grid',gridTemplateColumns:'38px minmax(0,1fr) auto',gap:10,alignItems:'start',padding:11,border:'1px solid var(--b1)',borderRadius:7,background:'var(--s2)'},announcementItem:{borderLeft:'3px solid #60a5fa'},deadlineItem:{borderLeft:'3px solid #f59e0b'},calendarIcon:{display:'grid',placeItems:'center',width:36,height:36,borderRadius:6,background:'rgba(96,165,250,.1)',color:'#93c5fd'},calendarContent:{display:'grid',gap:7,minWidth:0},calendarHeader:{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:8,flexWrap:'wrap'},calendarSource:{padding:'3px 7px',borderRadius:10,background:'rgba(148,163,184,.1)',color:'var(--muted2)',fontSize:9.5,textTransform:'uppercase'},calendarActions:{display:'flex',gap:5},
